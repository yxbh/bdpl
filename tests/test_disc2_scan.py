...
@pytest.fixture
def disc2_path() -> Path:
    """Return path to bundled disc2 fixture if available."""
...
class TestChapterSplitting:
    def test_finds_four_episodes(self, disc2_path: Path) -> None:
        pls = parse_mpls_dir(disc2_path / "PLAYLIST")
        clips = parse_clpi_dir(disc2_path / "CLIPINF")
        da: DiscAnalysis = scan_disc(disc2_path, pls, clips)
        assert len(da.episodes) == 4

    def test_episode_durations_reasonable(self, disc2_path: Path) -> None:
        pls = parse_mpls_dir(disc2_path / "PLAYLIST")
        clips = parse_clpi_dir(disc2_path / "CLIPINF")
        da: DiscAnalysis = scan_disc(disc2_path, pls, clips)
    ...
    def test_episodes_are_ordered(self, disc2_path: Path) -> None:
        pls = parse_mpls_dir(disc2_path / "PLAYLIST")
        clips = parse_clpi_dir(disc2_path / "CLIPINF")
        da: DiscAnalysis = scan_disc(disc2_path, pls, clips)
    ...
    def test_episode_segments_dont_overlap(self, disc2_path: Path) -> None:
        pls = parse_mpls_dir(disc2_path / "PLAYLIST")
        clips = parse_clpi_dir(disc2_path / "CLIPINF")
        da: DiscAnalysis = scan_disc(disc2_path, pls, clips)
