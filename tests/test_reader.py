...
class TestReadBytesAndString:
    ...
    def test_read_string_strips_nulls(self):
...
class TestCursor:
    ...
    def test_tell_advances_after_read(self):
        r = BinaryReader(b"\x00" * 10)
        r.u8()
        assert r.tell() == 1
        r.u16()
...
class TestSlice:
    ...
    def test_slice_independent_of_parent(self):
        r = BinaryReader(b"\x00\x01\x02\x03\x04\x05")
        child = r.slice(1, 2)
        child.u8()
...
class TestGuards:
    ...
    def test_require_raises_when_not_enough_bytes(self):
        r = BinaryReader(b"\x00\x01")
        with pytest.raises(ValueError, match="need 5 bytes"):
            r.require(5)

    def test_require_ok_when_enough_bytes(self):
        r = BinaryReader(b"\x00\x01\x02")
        r.require(3)  # Should not raise

    def test_require_at_raises_for_out_of_bounds(self):
        r = BinaryReader(b"\x00\x01\x02")
        with pytest.raises(ValueError):
            r.require_at(2, 5)

    def test_require_at_ok_within_bounds(self):
        r = BinaryReader(b"\x00\x01\x02\x03")
...
class TestRemaining:
    ...
    def test_remaining_after_reads(self):
        r = BinaryReader(b"\x00\x01\x02\x03")
        r.u8()
